...
@article1 = Article.create name: "Robotic Chicken", description: "The first robotic chicken was created last week in Geneva, by a group of two scientists that were cloned from stem-cells and the rump of a large cappola pig. Some critics are skeptical of the results, saying that they are 'biased' toward pigs and not fair enough to cows, but others agree with Mr. Anderson.", spree_user_id: @user.id, image: File.open(File.join(Rails.root, "db", "seeds", "images", "image1.jpg")), featured: true


@article2 = Article.create name: "Prototype Airplane", description: "A robotic airplane is being built that will traverse up to 200,000 miles of air in one charge and with no human pilot. The airplane is made of hydrogen which is actually impossible, but that didn't stop two brothers from the University of Jamaica from doing just that..", spree_user_id: @user.id, image: File.open(File.join(Rails.root, "db", "seeds", "images", "image2.jpg")), featured: true


@article3 = Article.create name: "The Robo-boogie", description: "If a robot boogies in the forest and there is no one there to see it, does it really happen? Researchers in Mississippi don't think so. For the love of Jermaine, why would you dress your friend up like a robot? The answer lies within, the robot suit - it's on a piece of paper.", spree_user_id: @user.id, image: File.open(File.join(Rails.root, "db", "seeds", "images", "image3.jpg")), featured: true
