...
@article1 = Article.create title: "Robotic Chicken", body: "The first robotic chicken was created last week in Geneva, by a group of two scientists that were cloned from stem-cells and the rump of a large cappola pig. Some critics are skeptical of the results, saying that they are 'biased' toward pigs and not fair enough to cows, but others agree with Mr. Anderson.", user_id: @user.id, image: File.open(File.join(Rails.root, "db", "seeds", "images", "image1.jpg")), featured: true


@article2 = Article.create title: "Prototype Airplane", body: "A robotic airplane is being built that will traverse up to 200,000 miles of air in one charge and with no human pilot. The airplane is made of hydrogen which is actually impossible, but that didn't stop two brothers from the University of Jamaica from doing just that..", user_id: @user.id, image: File.open(File.join(Rails.root, "db", "seeds", "images", "image2.jpg")), featured: true


@article3 = Article.create title: "The Robo-boogie", body: "If a robot boogies in the forest and there is no one there to see it, does it really happen? Researchers in Mississippi don't think so. For the love of Jermaine, why would you dress your friend up like a robot? The answer lies within, the robot suit - it's on a piece of paper.", user_id: @user.id, image: File.open(File.join(Rails.root, "db", "seeds", "images", "image3.jpg")), featured: true


@tutorial1 = Tutorial.create name: "How to build a robot", description: "This is the first test tutorial. It is not going to teach you anything, but it will give me something to read and look at while I am developing my web site. If you do not agree with this tutorial, that's too bad. This is the end of the description", image: File.open(File.join(Rails.root, "db", "seeds", "images", "image6.jpg"))
...
@tutorial2 = Tutorial.create name: "Beginning Electronics", description: "This is the first test tutorial. It is not going to teach you anything, but it will give me something to read and look at while I am developing my web site. If you do not agree with this tutorial, that's too bad. This is the end of the description", image: File.open(File.join(Rails.root, "db", "seeds", "images", "image7.jpg"))
...
@tutorial3 = Tutorial.create name: "Building a deck", description: "This is the first test tutorial. It is not going to teach you anything, but it will give me something to read and look at while I am developing my web site. If you do not agree with this tutorial, that's too bad. This is the end of the description", image: File.open(File.join(Rails.root, "db", "seeds", "images", "image10.jpg"))
